...
define("@scom/scom-mixed-chart", ["require", "exports", "@ijstech/components", "@scom/scom-mixed-chart/global/index.ts", "@scom/scom-mixed-chart/index.css.ts", "@scom/scom-mixed-chart/assets.ts", "@scom/scom-mixed-chart/data.json.ts"], function (require, exports, components_3, index_1, index_css_1, assets_1, data_json_1) {
            ...
            if (groupBy && item[groupBy] !== undefined) {
                ...
                _series = keys.map(v => {
                    ...
                    return {
                        ...
                        type: index_1.getChartType((seriesOpt === null || seriesOpt === void 0 ? void 0 : seriesOpt.type) || globalSeriesType, 'line'),
            ...
            const _chartData = {
                tooltip: {
                    ...
                    formatter: (params) => {
                        ...
                        if (_series.length === 1) {
                            res += `<div style="display: flex; justify-content: space-between; gap: 10px"><span>${params[0].marker} ${params[0].seriesName}</span> ${params[0].value[1] === null ? '-' : percentage ? index_1.formatNumber(params[0].value[1], { percentValues: true }) : index_1.formatNumberByFormat(params[0].value[1], labelFormats[params[0].seriesName])}</div>`;
                        }
                        else {
                            for (const param of params) {
                                if (param.value[1] !== null) {
                                    res += `<div style="display: flex; justify-content: space-between; gap: 10px"><span>${param.marker} ${param.seriesName}</span> ${percentage ? index_1.formatNumber(param.value[1], { percentValues: true }) : index_1.formatNumberByFormat(param.value[1], labelFormats[param.seriesName])}</div>`;
                                }
                            }
                        }
                        return res;
    ...
    ScomMixedChart = __decorate([
        ...
        components_3.customElements('i-scom-mixed-chart')
    ], ScomMixedChart);
